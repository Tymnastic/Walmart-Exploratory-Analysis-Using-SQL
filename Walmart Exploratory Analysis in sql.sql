...
WITH HourSales AS(
	...
	Invoice_id,
	Total,
	CONVERT(VARCHAR, DATEADD(MINUTE, CASE WHEN DATEPART(MINUTE, Time) < 30 THEN 0 ELSE 60 END - DATEPART(MINUTE, Time), Time), 108) AS TIMEEE
FROM
	Invoice
)
SELECT
	TIMEEE,
	count(TIMEEE) AS "Hourly Sales"
FROM 
	HourSales
GROUP BY
	TIMEEE
